...
function ImportCoupon(props) {
  ...
  useEffect(() => {
    setTotalValue(
      importProducts.reduce(
        (prev, current) => prev + current.sp_sl * current.sp_gianhap,
        0
      )
    );
  }, [importProducts]);
  ...
  function addCoupon() {
    axios
      .post('http://localhost:5000/api/pn/', {
  ...
  const addChiTietPn = (pn) => {
    ...
    axios
      .post('http://localhost:5000/api/ctpn/', ctpnp, {
    ...
    <div className="boder-main">
      ...
      <div>
        ...
        <table className="table-export table table-bordered align-middle ">
                  ...
                  <tr>
                    ...
                    <td>
                      <InputGroup size="sm" className="mb-3">
                        <InputGroup.Text id="nputGroup-sizing-sm">
                          <Icon icon={faEdit} />
                        </InputGroup.Text>
                        <Form.Control
          ...
          <tfoot>
            <tr>
              <td colSpan="6" className="text-start h5 fw-bold">
                SUM:{' '}
              </td>
          ...
          {products !== undefined && products.length !== 0 ? (
            products.map((item, i) => (
              <div key={i} className="d-flex">
                <button disabled className="fw-bold text-dark">
                  {i + 1}
                </button>
                <button
