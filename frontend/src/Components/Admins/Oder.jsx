...
function Oder(props) {
  ...
  const curentAccount = localStorage['admin']
  ...
  const onChange = (e) => {
    const temp = filterBill.filter((element) =>
      element.user_name.toLowerCase().includes(e.target.value.toLowerCase())
    );
    setBill(temp);
  };
        ...
        <>
          <td>
            <button
              ...
              onClick={() => updateStatus(id, 1, curentAccount.nv_id)}
            >
              <Icon icon={faCheckDouble} />
            </button>
          </td>
          <td>
            <button
              ...
              onClick={() => updateStatus(id, 3, curentAccount.nv_id)}
            >
              <Icon icon={faCircleXmark} />
            </button>
          </td>
      ...
      return (
        <>
          ...
          <td>
            <button
              className="btn btn-outline-danger"
              onClick={() => updateStatusOrder(id, 3)}
            >
              <Icon icon={faCircleXmark} />
            </button>
          </td>
  ...
  async function updateStatus(id, status, idAccount) {
    await axios
      .put(`http://localhost:5000/api/dh/${id}`, {
        dh_id: id,
        dh_status: status,
        nv_id: idAccount,
      })
  ...
  async function updateStatusOrder(id, status, idAccount) {
    await axios
      .delete(`http://localhost:5000/api/dh/${id}`, {
        dh_id: id,
        dh_status: status,
        nv_id: idAccount,
      })
  ...
  function renderStatus(status) {
    if (status === 2) {
      return <td className="text-success fw-bold">Has received</td>;
    } else if (status === 3) {
      return <td className="text-danger fw-bold">Deleted</td>;
    } else if (status === 0) {
      return <td className="text-warning fw-bold">Wait</td>;
    } else {
      return <td className="text-primary fw-bold">On Shipping</td>;
    }
  }
  ...
  return (
    <div className="boder-main">
      <ToastContainer />
      <h2 className="text-uppercase text-center text-primary fw-bolder mt-2">
        Order Management
      </h2>
      <h3 className="text-uppercase text-start text-success fw-bolder mx-2">
        filter <Icon icon={faFilter} />
      </h3>
      <Row className="m-0">
        <Col xs={12} md={2}>
          <h4 style={{ color: 'tomato', fontWeight: 'bolder' }}>
            All Orders{' '}
            <input
              type="checkbox"
              onChange={() => {
        ...
        <div style={{ overflowX: 'scroll', whiteSpace: 'nowrap' }}>
          <table className="table table-bordered">
            ...
            {bill !== undefined && bill.length !== 0 ? (
                ...
                {dataPage.map((value, idx) => {
                  return [
                    value.ctdh.map((item, i) => {
                      return (
                        <tr key={i}>
                          ...
                          <td>
                            {value.dh_address !== undefined
                              ? value.dh_address
                              : value.dh_address}
                          </td>
                          <td>
                            {value.user_phone !== undefined
                              ? value.user_phone
                              : value.user_phone}
                          </td>
                          <td>{value.dh_pay}</td>
                    ...
                    <tr key={idx} className="table-secondary">
                      <td
                        colSpan={3}
                        className="fw-bolder text-uppercase text-end"
                      >
                        Sum quantity:
                      </td>
                      <td
                      ...
                      <td
                        className="fw-bolder text-primary text-center"
                        colSpan={1}
                      >
                        {new Intl.NumberFormat('vi').format(value.dh_total)} $
                      </td>
              ...
              <tbody>
                <tr className="text-center fw-bolder text-danger h3">
                  <td colSpan={15}>Right now not have any orders</td>
                </tr>
              </tbody>
            )}
          </table>
      ...
      <Modal
        show={showListAccount !== false}
        onHide={() => setShowListAccount(false)}
      >
          ...
          {listAccount !== undefined && listAccount?.length !== 0 ? (
            listAccount?.map((item, i) => (
              <div key={i} className="d-flex">
                <button disabled className="text-dark fw-bold">
                  {i + 1}
                </button>
                <button
                  className="border-0 btn btn-outline-info text-dark w-100 text-start m-1"
                  onClick={() => {
                    updateStatus(showListAccount, 1, item.nv_id);
