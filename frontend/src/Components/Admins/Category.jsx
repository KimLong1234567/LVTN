...
function Category(props) {
  ...
  const deleteBrand = (id) => {
    axios.delete(`http://localhost:5000/api/cate/${id}`).then((res) => {
  ...
  const updateType = async (id) => {
    ...
    await axios
      .put(
        `http://localhost:5000/api/cate/${id}`,
        formdata,
        {},
        {
          headers: { 'Content-Type': 'multipart/form-data' },
  ...
  const TypeFind = (e) => {
    const temp = typeFind.filter((element) =>
      element.cate_name.toLowerCase().includes(e.target.value.toLowerCase())
    );
    console.log(temp);
    setType(temp);
  };
    ...
    <div className="boder-main">
      ...
      <div className="d-flex justify-content-between px-3 pb-2">
        ...
        <Button variant="success" onClick={() => setShow(true)}>
          Add new category <Icon icon={faSquarePlus} />
        </Button>
        <Modal show={show !== false} onHide={() => setShow(false)} id="edit">
            ...
            <Form>
              <Form.Group className="mb-3" controlId="formBasicEmail">
                ...
                <Form.Control
                  ...
                  name="cate_name"
                  placeholder={
                    show.cate_name ? show.cate_name : 'Category Name'
                  }
                  onChange={onChange}
                />
              </Form.Group>
          ...
          <Modal.Footer>
            {show.cate_id !== undefined ? (
              <Button
                variant="primary"
                onClick={() => updateType(show.cate_id)}
              >
                Update
              </Button>
            ) : (
              ...
              <tr key={i}>
                ...
                <td>
                  <Form.Check
                    ...
                    id="custom-switch"
                    onChange={() => {
                      statusType(Type.cate_id, Type.c_status);
                    }}
                    checked={Type.c_status === 1 ? true : false}
                  />
                </td>
                <td className="text-center">
                  <button
                ...
                <td className="text-center">
                  <button
                    ...
                    onClick={() => deleteBrand(Type.cate_id)}
                  >
