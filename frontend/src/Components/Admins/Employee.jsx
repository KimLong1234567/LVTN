...
function Employee(props) {
  ...
  const deleteAccount = (id) => {
    axios.delete(`http://localhost:5000/api/admins/${id}`).then((res) => {
  ...
  const findaccount = (e) => {
    const temp = FindAccount.filter((element) =>
      element.nv_hoten.toLowerCase().includes(e.target.value.toLowerCase())
    );
    setAccounts(temp);
  };
  return (
    <div className="boder-main">
      ...
      <div className="d-flex justify-content-between m-3">
        ...
        <Button variant="success" onClick={() => setShow(true)}>
          Add employee account <Icon icon={faSquarePlus} />
        </Button>
        <Modal show={show !== false} onHide={() => setShow(false)} id="edit">
            ...
            <Form>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="text"
                  name="nv_hoten"
                  placeholder={show.nv_hoten ? show.nv_hoten : 'Name'}
                  onChange={onChange}
                />
              </Form.Group>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="text"
                  name="nv_email"
                  placeholder={show.nv_email ? show.nv_email : 'Email'}
                  onChange={onChange}
                />
              </Form.Group>
              <Form.Group className="mb-3">
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="text"
                  name="nv_phone"
                  placeholder={show.nv_phone ? show.nv_phone : 'Phone'}
                  onChange={onChange}
                />
              </Form.Group>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="text"
                  name="nv_address"
                  placeholder={show.nv_adress ? show.nv_adress : 'Address'}
                  onChange={onChange}
                />
              </Form.Group>
              <Form.Group className="mb-3">
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Select
                  aria-label="Default select example"
                  name="cv_id"
                  onChange={handlCvChange}
                >
                  <option>-- Choose Position --</option>
                  {cate.map((category) => (
                    <option key={category.cv_id} value={category.cv_id}>
                      {category.cv_name}
                    </option>
                  ))}
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="text"
                  name="nv_date"
                  placeholder={show.nv_date ? show.nv_date : 'dd/mm/yyyy'}
                  onChange={onChange}
                />
              </Form.Group>
          ...
          <Modal.Footer>
            {show.nv_id !== undefined ? (
              <Button
                variant="primary"
                onClick={() => updateAccount(show.nv_id)}
              >
                Update
              </Button>
            ) : (
              ...
              <tr key={i}>
                ...
                <td className="text-center">
                  <button
                    ...
                    onClick={() => deleteAccount(account.nv_id)}
                  >
