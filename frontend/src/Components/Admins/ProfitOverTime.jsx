    ...
    axios.get('http://localhost:5000/api/dh/dhang/success').then((res) => {
      ...
      res.data?.data?.forEach((item) => {
        newBill.push({
          ...item,
          dh_create: format(
            parse(item.dh_create, "yyyy-MM-dd'T'HH:mm:ss.SSSX", new Date()),
            'yyyy-MM-dd'
          ),
        });
      });
          ...
          {Bill.length > 0 ? (
            Bill.map((value, idx) => (
              <tbody key={idx}>
                ...
                <tr className="table-secondary text-center">
                  <td colSpan={4} className="fw-bolder text-uppercase text-end">
                    Sum:
                  </td>
                  <td className="fw-bolder text-uppercase text-center">
                    {value.dh_sl}
                  </td>
            ...
            <div className="d-flex">
              <h5 className="m-3 text-warning">
                Orders have not yet been sold.
              </h5>
            </div>
          )}
        </table>
