...
function ExportPDF(props) {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/dh/dhang/success/${id}`
        );
  ...
  console.log(bill);
  let ngaydat = bill[0]?.dh_create
    ? new Date(bill[0].dh_create).toLocaleDateString()
    : '';
  let ngaynhan = bill[0]?.dh_create
    ? new Date(bill[0].dh_create).toLocaleDateString()
    : '';
  const exportPDF = useReactToPrint({
    ...
    onAfterPrint: () =>
      toast.success('File exporting is successful.', {
        autoClose: 2000,
        closeOnClick: true,
        pauseOnHover: true,
        draggable: true,
            ...
            <Row>
              ...
              <Col sm={5} className="text-center">
                ...
                <p className="fw-bold h5">
                  Cần Thơ, Date {date} month {month + 1} years {years}
                </p>
              </Col>
              <Col sm={4} className="text-start text-danger mt-3">
                <h5>
                  Date Order:{' '}
                  <span className="p-0 m-0 text-dark">{ngaydat}</span>
                </h5>
                <h5>
                  Date of Delivery:{' '}
                  <span className="p-0 m-0 text-dark">{ngaynhan}</span>
                </h5>
                <h5>
                  No: <span className="p-0 m-0 text-dark">{id}</span>
                </h5>
              </Col>
            </Row>
            <Row>
            ...
            <div className="text-start">
              ...
              <Row className="text-center">
                <h5 className="text-primary">
                  Sum quantity: {bill[0]?.dh_sl}{' '}
                </h5>
                <h5 className="text-primary">
                  The total amount due:{' '}
                  {new Intl.NumberFormat('vi').format(bill[0]?.dh_total)} $
                </h5>
                <Col sm={4}>
                  <h5>Customer</h5>
                ...
                >
                  (A dedication to abiding by legal requirements and sample
                  standards for printing at retail establishments.)
                </h6>
              </Row>
            </div>
          </div>
          <Button className="m-2" variant="primary" onClick={() => exportPDF()}>
            {' '}
            Export file{' '}
          </Button>
        </Container>
