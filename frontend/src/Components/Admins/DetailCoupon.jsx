...
function DetailCoupon(props) {
  ...
  useEffect(() => {
    if (id != null) {
      axios.get(`http://localhost:5000/api/pn/ctpn/${id}`).then((res) => {
  ...
  return (
    <div className="boder-main">
      <div className="mx-2">
        <h2 className="text-center fs-4 fw-bold py-3 ">DETAILS COUPON</h2>
        <Row>
          <Col md={4}>
            <p className="fw-bold">
              No: <span className="text-danger ">{coupon?.pn_id}</span>
            </p>
          </Col>
          <Col md={4}>
            {coupon.pn_create !== undefined ? (
              <p className="fw-bold">
                ...
                <span className="text-primary">
                  {new Date(coupon.pn_create).toLocaleString()}
                </span>
              </p>
            ) : null}
          </Col>
        ...
        <Row>
          <Col md={3}>
            <p className="fw-bold">
              Staff: <span className="text-primary">{coupon?.nv_hoten}</span>
            </p>
          </Col>
          <Col md={2}>
            <p className="fw-bold">
              Staff code: <span className="text-primary">{coupon?.nv_id}</span>
            </p>
          </Col>
          <Col md={3}>
            <p className="fw-bold">
              Staff's Phone:{' '}
              <span className="text-primary">{coupon?.nv_phone}</span>
            </p>
          </Col>
          <Col md={4}>
            <p className="fw-bold">
              Email: <span className="text-primary">{coupon?.nv_email}</span>
            </p>
          </Col>
        </Row>
        <table className="table-export table table-bordered align-middle">
          ...
          <tfoot>
            <tr className="table-secondary">
              <td colSpan="5" className="text-start h5 fw-bold">
                SUM:{' '}
              </td>
              <td colSpan="4" className="text-end h5 fw-bold">
                {' '}
                {coupon?.pn_total} $
              </td>
            </tr>
