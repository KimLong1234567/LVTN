...
function Cart(props) {
    ...
    if (text === 'Pay online') {
      return (
        <div>
          ...
          >
            Pay <Icon icon={faAmazonPay} />{' '}
          </Button>
          <PayPalScriptProvider
            options={{
              'client-id':
                'ARAvyA7MJ6zO-uJ3WW7w5ypwzPboZ1hkDsEE8ZJm4jcwoYB7DdSxTQ-YUER58nunHYF8eWAkToPnFfGC',
            }}
          >
            {show ? (
              <PayPalButtons
            ...
            {success ? <h3 className="text-success">pay success</h3> : null}
            {orderID ? <h5> Order ID: {orderID}</h5> : null}
  ...
  useEffect(
    () => {
      axios
        .get(`http://localhost:5000/api/dh/${curentAccount.user_id}`)
        .then((res) => {
  ...
  useLayoutEffect(() => {
    async function fecthData() {
      const res = (
        await axios.get(
          `http://localhost:5000/api/orders/user/orders/${curentAccount.user_id}`,
          {
        ...
        if (products[i].gh_id === id) {
          ...
          axios
            ...
            .then((res) => {
              ...
              setTimeout(() => setRefresh((prev) => prev + 1), 3000);
              console.log(refresh);
            });
        }
      }
  ...
  const updateQuantity = (product, amount) => {
    ...
    const productIndex = products.findIndex(
      (p) => p.gh_id === updatedProduct.gh_id
    );
  ...
  const createBillData = (totalBillValue, totalBillUnit) => {
    const dh_pay = infoBill.dh_pay;
    const user_id = curentAccount ? curentAccount.user_id : '';
    const totalValue = totalBillValue;
    const totalUnit = totalBillUnit;
  ...
  function sendBillDataToServer() {
    ...
    axios
      .put(
        `http://localhost:5000/api/users/${curentAccount.user_id}`,
        infoBill.user_phone
      )
      .then();
    axios.post('http://localhost:5000/api/dh/', dh).then((res) => {
      const user_phone = infoBill.user_phone;
      const dh = res.data.data;
      console.log(res.data.data, dh);
      addChiTietDh(dh);
      axios.put(`http://localhost:5000/api/users/${curentAccount.user_id}`, {
        user_phone,
      });
  ...
  const addChiTietDh = (dh) => {
    ...
    axios
      .post('http://localhost:5000/api/ctdh/', ctdh, {
        ...
        <Col xs={4} sm={3}>
          <div className="bg-light text-center ">
            <h4 className="text-uppercase text-success">
              {' '}
              <Icon icon={faCaretRight} className="m-1" />
              Customer Info{' '}
            </h4>
            <div className="text-start mx-3 pb-2">
              <Form>
                ...
                >
                  ...
                  <Form.Control
                    type="text"
                    placeholder={curentAccount.user_name}
                    name="name_customer"
                    disabled
                    required
                  />
              ...
              <Form>
                ...
                >
                  ...
                  <Form.Control
                    type="email"
                    value={curentAccount.user_email}
                    disabled
                  />
                </Form.Group>
              </Form>
              <Form>
                ...
                >
                  ...
                  {curentAccount.user_phone ? (
                    <Form.Control
                      type="text"
                      placeholder={curentAccount.user_phone}
                      name="user_phone"
                    />
                  ) : (
                    <Form.Control
        ...
        <Col xs={8} sm={9}>
                        ...
                        <tr>
                          ...
                          <td>
                            <button
                              ...
                              onClick={() => deleteProducts(item.gh_id)}
                            >
          ...
          <Row>
            <div className="text-end g-3">
              <h4>
                Sum quantity: {new Intl.NumberFormat('vi').format(totalUnit)}{' '}
                unit
              </h4>
