...
function ResetPasswordCustomer(props) {
  ...
  function onSumit() {
    ...
    axios
      .post(
        `http://localhost:5000/api/users/reset-password/${Token.email}`,
        password
      )
      .then((res) => {
    ...
    <Container className="padding-header d-fex">
      ...
      <div style={{ width: '500px' }} className="mx-auto">
        <Form className="text-start">
          <h2 className="text-center text-primary">Update your password</h2>
          <Form.Group>
            <Form.Label htmlFor="username" className="control-label fw-bold">
              {' '}
              Password
            </Form.Label>
            <Form.Control
