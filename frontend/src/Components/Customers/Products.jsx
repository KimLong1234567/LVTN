...
function Products(props) {
  ...
  function PaginatedItems({ itemsPerPage }) {
    ...
    return (
      <Row>
        {dataPage.length === 0 ? (
          <h4 className="text-danger fw-bold">No Products found</h4>
        ) : (
          dataPage.map((item, idx) => renderProducts(item, idx))
        )}
        <ReactPaginate
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/cate/').then((res) => {
      // console.log(res);
      ...
      return (
        <Col xs={12} md={6} sm={6} key={idx} className="g-3">
          <Card className="card cardProduct border-success border border-2">
            ...
            <Card.Body>
              <Card.Title className="text-uppercase mt-3">
                {item.sp_name}
              </Card.Title>
            ...
            >
              <Card.Title className="footer-card">Details</Card.Title>
            </Link>
          </Card>
        </Col>
      );
    ...
    if (item.c_status === 1) {
      return (
        <h5 key={idx} className="m-1 text-start text-uppercase type">
          <Link
            className=" text-dark"
            onClick={() => ProductType(item.cate_id)}
          >
            <img
              src={`/image/Loai/${item.cate_img}`}
              alt="..."
              style={{ width: '100px' }}
              className="m-3"
            />
  ...
  const setPrice = (e) => {
    const temp = ProductFind.filter(
      (element) => element.sp_price < e.target.value
    );
    setProducts(temp);
  };
  ...
  useEffect(() => {
    if (sort === 'Từ thấp đến cao') {
      axios.get('http://localhost:5000/api/products/low/a').then((res) => {
        const temp = res.data.data;
        setProducts(temp);
      });
    } else if (sort === 'Từ cao đến thấp') {
      axios.get('http://localhost:5000/api/products/high/b').then((res) => {
        // console.log(res);
        const temp = res.data.data;
        setProducts(temp);
      });
    ...
    <div>
      ...
      <Container fluid>
        <Row className="d-flex mt-4 pb-4 justify-content-center">
          <div className="d-flex w-50">
            <input
              ...
              // value={searchValue}
            />
            <Button variant="outline-success" id="voice" className="text-start">
              {' '}
        ...
        <Row>
          <Col xs={3}>
            <div
              className="text-start bg-light p-3 "
              style={{
                position: 'sticky',
                zIndex: 1000,
                fontFamily: 'cursive',
            ...
            >
              <h4
                style={{ color: 'rgb(255,142,81)', textTransform: 'uppercase' }}
              >
                {' '}
                <Icon icon={faCaretRight} className="me-1 mt-1" />
                Shop by pet
              </h4>
              {type.map((item, idx) => renderType(item, idx))}
            </div>
            <div
              className="text-start bg-light mt-5 p-3"
          ...
          <Col xs={9}>
            <Row className="d-flex">
              <h2 className="bg-light text-start text-info fw-bold py-2 text-uppercase">
                {' '}
                <Icon icon={faCaretRight} className="me-2 mt-1" />
                All Products
              </h2>
            </Row>
            <Row>
