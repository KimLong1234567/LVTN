...
function Bill(props) {
  ...
  const curentAccount = localStorage['currentAccount']
    ? JSON.parse(localStorage['currentAccount'])
    : null;
  useLayoutEffect(() => {
    async function fetchData() {
      const res = await axios.get(
        `http://localhost:5000/api/dh/dh/${curentAccount.user_id}`
      );
  ...
  function addFeedback() {
    ...
    axios
      .post('http://localhost:5000/api/contacts/', feedbackData)
      .then((res) => {
        toast.success(
          "You've responded; please check your email for more details.",
          {
            position: 'top-center',
            autoClose: 2000,
            closeOnClick: true,
          ...
          <Form>
            ...
            >
              <Form.Control
                as="textarea"
                rows={4}
                placeholder="Please give us feedback about your order"
                name="lh_content"
                onChange={onchange}
              />
            </Form.Group>
        ...
        {bill !== undefined && bill.length !== 0 ? (
          <tbody>
            {dataPage.map((value, idx) => {
              ...
              return [
                  ...
                  return (
                    <tr key={i}>
                      ...
                      <td>
                        {value.nv_hoten !== null
                          ? value.nv_hoten
                          : 'There is currently no delivery person'}
                      </td>
                      <td>
                        {value.nv_phone !== null ? value.nv_phone : 'Empty'}
                      </td>
                      <td colSpan={3}></td>
                    </tr>
                  );
                ...
                <tr key={idx}>
                  <td colSpan={3} className="fw-bolder text-uppercase text-end">
                    Sum quantity:
                  </td>
                  <td
                    colSpan={1}
                  ...
                  <td colSpan={8} className="fw-bolder text-uppercase text-end">
                    Sum:{' '}
                  </td>
                  <td className="fw-bolder text-primary text-end" colSpan={1}>
          ...
          <tbody>
            <tr className="text-center fw-bolder text-danger h3">
              <td colSpan={12}>Not have bill yet</td>
            </tr>
          </tbody>
        )}
      </table>
