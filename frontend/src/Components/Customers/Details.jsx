...
function Details(props) {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/products`).then((res) => {
      setProducts(
        res.data.data.filter((e) => e.type?.cate_id === Product.type?.cate_id)
      );
    });
  ...
  return (
    <Container fluid className="padding-header">
        ...
        <Row>
          <Col xs={12} sm={7}>
            <div className="container">
              <img
                src={`/image/SanPham/${Product[0].sp_image}`}
                className="w-100"
                alt="..."
                style={{ height: '600px' }}
              />
            ...
            <div className="text-start mx-5 mt-1">
              <h4>
                Comments <Icon icon={faCommentSms} />{' '}
              </h4>
              {comments.length !== 0
                ? comments.map((item, i) => {
                    return (
          ...
          <Col xs={12} sm={5}>
            <div className="mx-5">
              ...
              <Table>
                <tbody>
                  ...
                  <tr>
                    <td className="text-start fw-bold text-uppercase">
                      Price:
                    </td>
                    <td className="text-end">
                      {new Intl.NumberFormat('vi').format(Product[0].sp_price)}{' '}
                      $
                    </td>
                  </tr>
                  <tr>
            ...
            <div className="comment-Products mx-5 col-10">
              <Form>
                ...
                >
                  <Form.Label>
                    Comment's product <Icon icon={faCommentDots} />
                  </Form.Label>
                  <Form.Control
                    as="textarea"
                    rows={4}
      ...
      <Container className="mt-2">
        <Row>
          <h4 className="text-start text-uppercase text-danger">
            Same products:
          </h4>
          {Products.map((item, idx) => {
            return (
              item.cate_id === Product[0].cate_id && (
