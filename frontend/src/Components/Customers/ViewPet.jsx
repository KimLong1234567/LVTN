...
function ViewPet(props) {
  ...
  useLayoutEffect(() => {
    async function fetchData() {
      const res = await axios.get(
        `http://localhost:5000/api/pets/userPet/${curentAccount.user_id}`
      );
  ...
  async function updateStatus(id, status) {
    await axios
      .put(`http://localhost:5000/api/pets/${id}`, {
        id: id,
        p_status: status,
      })
      .then((res) => {
  ...
  function renderStatus(p_status) {
    if (p_status === 1) {
      return <td className="text-success fw-bold">finish service</td>;
    } else if (p_status === 3) {
  ...
  function addFeedback() {
    ...
    axios
      ...
      .then((res) => {
        toast.success(
          'Responded, please check your email to receive information',
          {
            position: 'top-center',
            autoClose: 2000,
            closeOnClick: true,
          ...
          <Form>
            ...
            >
              <Form.Control
                as="textarea"
                rows={4}
                placeholder="Please let us know what you think of our service."
                name="lh_content"
                onChange={onchange}
              />
            </Form.Group>
        ...
        {pet !== undefined && pet.length !== 0 ? (
          <tbody>
            {pet.map((item, i) => (
              <tr key={i}>
                ...
                <td>
                  {(item.p_create = new Date(item.p_create).toLocaleString())}
                </td>
                <td>{item.service_price} $</td>
                <td>
                  {item.p_update === null
                    ? 'till waitting'
                    : (item.p_update = new Date(
                        item.p_update
                      ).toLocaleString())}
                </td>
          ...
          <tbody>
            <tr className="text-center fw-bolder text-danger h3">
              <td colSpan={12}>No pets in book pets</td>
            </tr>
          </tbody>
        )}
      </table>
